...
<div class="content-wrapper">
    ...
    for ($i=0; $i < count($res); $i++) { 
        ...
        $query = $pdo->query("SELECT * FROM user where cpf = '$cpf'");
        $res = $query->fetchAll(PDO::FETCH_ASSOC);
        ...
        $cpf_formatado_professor = substr_replace($cpf, '.', 3, 0);
        $cpf_formatado_professor  = substr_replace($cpf_formatado_professor, '.', 7, 0);
        $cpf_formatado_professor  = substr_replace($cpf_formatado_professor , '-', 11, 0);

        $aniversario_formatado = date('d/m/Y', strtotime($aniversario));

        $admissao_formatado = date('d/m/Y', strtotime($admissao));
    ...
    <div class="ficha-aluno">
        <div class="conteiner-principal">
            ...
            <div class="foto" >
                <?php if ($imagem) { ?>
                    <img src="../img/funcionarios/<?php echo $imagem;?>" alt="Foto do Aluno">
                <?php } ?>
                </div>
            </div>
            <div class="conteiner">
        ...
        <table>
            <tr>
                <th colspan="2" style="text-align: center; text-transform: uppercase;">Ficha do Professor</th>
            </tr>

            <?php if($nivel != ''){  ?>
            <tr>
                <td> Nome: <?php echo $nomecompleto; ?></td>
                <td>Painel: <?php echo $nivel; ?></td>
            </tr>
            <?php } else{?>
                </tr>
            ...
            <tr>
                <td>CPF: <?php echo $cpf_formatado_professor; ?></td>
                <td>RG: <?php echo $rg_formatado; ?></td>
            </tr>
            <tr>
                <td> Cargo: <?php echo $cargo; ?></td>
                <td> Desde: <?php echo  $admissao_formatado; ?></td>
            </tr>
            <tr>
